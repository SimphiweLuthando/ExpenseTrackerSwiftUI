//
//  Category.swift
//  ExpenseTracker
//
//  Created by Simphiwe Mbokazi on 15/07/23.
//  

//

import Foundation
import SwiftUI

// Enum that defines all categories of expenses
enum Category: String, CaseIterable {
    case donation
    case food
    case entertainment
    case health
    case shopping
    case transportation
    case utilities
    case other
    
    // A computed property that provides the corresponding system icon for each category
    var systemNameIcon: String {
        switch self {
        case .donation: return "heart.circle.fill"   // Icon for donation category
        case .food: return "archivebox"              // Icon for food category
        case .entertainment: return "tv.music.note"  // Icon for entertainment category
        case .health: return "staroflife"            // Icon for health category
        case .shopping: return "cart"                // Icon for shopping category
        case .transportation: return "car"           // Icon for transportation category
        case .utilities: return "bolt"               // Icon for utilities category
        case .other: return "tag"                    // Icon for other category
        }
    }
    
    // A computed property that provides the corresponding color for each category
    var color: Color {
        switch self {
        case .donation: return Color(#colorLiteral(red: 0.8549019694, green: 0.250980407, blue: 0.4784313738, alpha: 1))
        case .food: return Color(#colorLiteral(red: 0.3647058904, green: 0.06666667014, blue: 0.9686274529, alpha: 1))
        case .entertainment: return Color(#colorLiteral(red: 0.5807225108, green: 0.066734083, blue: 0, alpha: 1))
        case .health: return Color(#colorLiteral(red: 0.9254902005, green: 0.2352941185, blue: 0.1019607857, alpha: 1))
        case .shopping: return Color(#colorLiteral(red: 0.9607843161, green: 0.7058823705, blue: 0.200000003, alpha: 1))
        case .transportation: return Color(#colorLiteral(red: 0.3411764801, green: 0.6235294342, blue: 0.1686274558, alpha: 1))
        case .utilities: return Color(#colorLiteral(red: 0.1764705926, green: 0.4980392158, blue: 0.7568627596, alpha: 1))
        case .other: return Color(#colorLiteral(red: 0.5791940689, green: 0.1280144453, blue: 0.5726861358, alpha: 1))
        }
    }
}

// Extension that makes Category Identifiable for use with SwiftUI
extension Category: Identifiable {
    var id: String { rawValue } // The rawValue is used as the id
}
